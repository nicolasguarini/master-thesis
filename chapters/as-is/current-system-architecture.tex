...
The SRP system architecture is fundamentally distributed into two primary segments: Company Network and Client Network.
...
The overall architecture is represented in the diagram in figure \ref{fig:as-is-system-design} \cite{service_request_portal}.
...
\begin{itemize}
    \item \textbf{SRP REST Web Sevice} \cite{srp_webservicerest_git}:  Deployed on a Kubernetes cluster, exposes .NET Core RESTful services \cite{service_request_portal}. It serves as the primary interface for external systems such as:
    \begin{itemize}
        \item \textbf{Delivery Portal}: An internal asset management system used to manage Service Request catalog configuration and company-managed servers and devices;
        \item \textbf{HDA (HelpDeskAdvanced)}: A ticket management system. Each service request is linked to an HDA ticket via the ticket ID, and HDA is responsible for initiating and associating service requests with existing tickets.
        \item \textbf{MyElmec}: A customer-facing portal enabling customers to create, manage, and monitor the various services provided by Elmec (e.g., servers, virtual machines, DaaS devices, cloud solutions, \dots).
    \end{itemize}
    The \textit{SRP REST Web Service} communicates with the \texttt{ELMEC-CMI-MSSQL} server.
...
The only component in the client's network is the \textbf{Domain Controller}, which represents the client's Active Directory domain controller and hosts the communication tools installed for interaction with Elmec. It hosts:
\begin{itemize}
